...
func TestCertificateSigner_Regenerate(t *testing.T) {
	...
	filesStruct := map[string]string{
		"":                                     CACertPath(filepath.Join(tmpDir, "root")),
		"test-client":                          ClientCertPath(filepath.Join(tmpDir, "root", "test-client")),
		"test-client2":                         ClientCertPath(filepath.Join(tmpDir, "root", "test-client2")),
		"test-server":                          ServingCertPath(filepath.Join(tmpDir, "root", "test-server")),
		"test-signer":                          CACertPath(filepath.Join(tmpDir, "test-signer")),
		"test-signer/test-signer-sub":          CACertPath(filepath.Join(tmpDir, "test-signer", "test-signer-sub")),
		"test-signer/test-signer-sub/sub-peer": PeerCertPath(filepath.Join(tmpDir, "test-signer", "test-signer-sub", "sub-peer")),
